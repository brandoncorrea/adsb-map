...
(def bare-mode-s-raw
  "A heard-but-never-positioned target: hex, messages, rssi, seen only."
  {:hex "a10202" :type "mode_s" :messages 74 :seen 3.5 :rssi -29.7})

(def mangled-raw
  "The shape of a corrupted burst: alt_baro is suddenly a nested map."
  (assoc cruising-raw :hex "badbad" :alt_baro {:corrupt true}))
...
(deftest ->aircraft
  ...
  (testing "coerces alt_baro of \"ground\" to an on-ground flag, not an
            altitude"
    (let [aircraft (coerce/->aircraft
                     (assoc cruising-raw :alt_baro "ground"))]
      (is (true? (:aircraft/on-ground? aircraft)))
      (is (not (contains? aircraft :aircraft/altitude-ft)))))

  (testing "keeps a never-positioned aircraft, without
            :aircraft/position"
  ...
  (testing "absent gs/track/baro_rate stay absent — never defaulted to
            zero"
    (doseq [[raw-field domain-key] {:gs :aircraft/ground-speed-kt
      ...
      (let [aircraft (coerce/->aircraft (dissoc cruising-raw raw-field))]
        (is (not (contains? aircraft domain-key))
            (str raw-field " absent must not become a zero")))))
  ...
  (testing "lower-cases the hex so aircraft identity is stable"
    (is (= "abc0e4"
           (:aircraft/icao (coerce/->aircraft
                             (assoc cruising-raw :hex "ABC0E4"))))))
  ...
  (testing "returns nil, without throwing, for junk that is not even a
            map"
  ...
  (testing "an absurd altitude costs the field, not the aircraft, and is
    ...
    (let [aircraft (coerce/->aircraft
      ...
      (is (= 450.5 (:aircraft/ground-speed-kt aircraft))
          "the rest of the aircraft's data survives")))
