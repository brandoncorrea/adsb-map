(ns adsb.map.crop-test
  (:require [adsb.fixtures :refer [captured-at-ms declared-crop]]
            [adsb.geo :as geo]
...
(defn- ring [fake]
  (get-in (first (test-map/last-features fake)) [:geometry :coordinates 0]))
...
(deftest boundary-reaches-the-map
  (testing "a config event carrying a crop puts the ring into the source"
    (rf-test/run-test-sync
      (let [fake (test-map/recording-map)]
  ...
  (testing "a crop-DISABLED deployment draws no boundary at all"
    (rf-test/run-test-sync
      (let [fake (test-map/recording-map)]
        ...
        (r/flush)
        (is (empty? (test-map/last-features fake))))))
  ...
  (testing "a SEEDED latch frames nothing either: this is the map a theme
    ...
    (rf-test/run-test-sync
      (let [fake (test-map/recording-map)]
        ...
        (r/flush)
        (is (empty? (fits fake)))
        (is (seq (test-map/last-features fake)))))))
